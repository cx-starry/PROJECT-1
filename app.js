...
.directive('loadProgress',[function(){
	return {
		replace:true,
		compile:function(tplele,tplattr,tranclude){
			if (tplele.length == 1){
				var node = tplele[0];
				...
				return {
					pre: function preLink(scope,instanceElement,instanceAttribute,controller){
...
angular.module('progress',['angular.directives-load-progress']).controller('ctrl_progress',function($scope,$interval){
	$scope.ProgressValue = {
		label : 0,
		percentage: 0.00
	}
